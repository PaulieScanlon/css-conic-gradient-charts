...
const Chart = ({ data }) => {
  ...
  const css_string = data
    .map((_, index, array) => {
      ...
      return ` var(--color-pink-${(array.length - index) * 100}) ${start_degrees}deg ${end_degrees}deg`;
    })
    .join();
  ...
  return (
    <div className='flex flex-col gap-8 grow'>
      <div className='flex flex-col grow'>
        ...
        <p className='m-0 text-xs text-gray-400 flex'>./components/donut-chart-clip-path-dynamic-colors.js</p>
      </div>
      <div>
